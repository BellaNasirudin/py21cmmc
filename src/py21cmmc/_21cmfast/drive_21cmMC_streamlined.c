...
struct ReturnData drive_21CMMC(char* arg1, char* arg2, float FlagOptions[], float AstrophysicalParams[], float CosmologicalParams[]) {
    ...
    DataToBeReturned.RandomVariables = calloc(6,sizeof(float));
    ...
    if(READ_FROM_FILE) {
        ...
        for(i=0;i<WALKER_FILE_LENGTH;i++) {
            if(i==0) {
                fscanf(F,"%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",&dummy_string,&GenerateNewICs,&SUBCELL_RSD,&USE_FCOLL_IONISATION_TABLE,&SHORTEN_FCOLL,&USE_TS_FLUCT,&INHOMO_RECO,&STORE_DATA,&PRINT_FILES,&PRINT_COEVAL_21cmBoxes,&PRINT_LIGHTCONE_21cmBoxes);
            }
    ...
    if(USE_LIGHTCONE) {
        ...
        DataToBeReturned.PSData = calloc(NUM_BINS*total_num_boxes,sizeof(float));
    }
    else {
        Co_eval_box_counter = 0;
        
        DataToBeReturned.PSData_k = calloc(NUM_BINS*N_USER_REDSHIFT,sizeof(float));
        DataToBeReturned.PSData = calloc(NUM_BINS*N_USER_REDSHIFT,sizeof(float));
    }
    
    if(USE_LIGHTCONE) {
        DataToBeReturned.LCBox = calloc((float)HII_DIM*(float)HII_DIM*(float)total_slice_ct,sizeof(float));
    }
    else {
        DataToBeReturned.LCBox = calloc(1,sizeof(float));
        DataToBeReturned.LCBox[0] = 12.0;
    }
...
//        printf( "The ST mean collapse fraction is %e, which is much smaller than the effective critical collapse fraction of %e\n I will just declare everything to be neutral\n", mean_f_coll_st, f_coll_crit);
        ...
        if(sample_index==0) {
            ...
            if(start_index_LC[N_USER_REDSHIFT-sample_index-1] > end_index_LC[N_USER_REDSHIFT-sample_index-1]) {
                ...
                if(num_boxes_interp==total_num_boxes) {
                    ...
                    for (ii=0;ii<remainder_LC; ii++){
                        for (j=0;j<HII_DIM; j++){
                            for (k=0; k<HII_DIM; k++){
                                DataToBeReturned.LCBox[(ii + total_num_boxes*HII_DIM) + (total_num_boxes*HII_DIM + remainder_LC)*(k+HII_DIM*j)] = box_interpolate_remainder[ii + remainder_LC*(k+HII_DIM*j)];
                            }
                        }
                    }
                    ...
                    for (ct=1; ct<NUM_BINS; ct++){
                        if (in_bin_ct[ct]>0) {
                            DataToBeReturned.PSData_k[ct+NUM_BINS*num_boxes_interp] = k_ave[ct]/(in_bin_ct[ct]+0.0);
                            DataToBeReturned.PSData[ct+NUM_BINS*num_boxes_interp] = p_box[ct]/(in_bin_ct[ct]+0.0);
                        }
                    }
    ...
    if(!USE_LIGHTCONE) {
        ...
        for (ct=1; ct<NUM_BINS; ct++){
            if (in_bin_ct[ct]>0) {
                // (N_USER_REDSHIFT - 1 - Co_eval_box_counter) to ensure the data is provided in increasing order of redshift (specific for the co-eval case)
                DataToBeReturned.PSData_k[ct+NUM_BINS*(N_USER_REDSHIFT - 1 - Co_eval_box_counter)] = k_ave[ct]/(in_bin_ct[ct]+0.0);
                DataToBeReturned.PSData[ct+NUM_BINS*(N_USER_REDSHIFT - 1 - Co_eval_box_counter)] = p_box[ct]/(in_bin_ct[ct]+0.0);
            }
        }
        Co_eval_box_counter += 1;
